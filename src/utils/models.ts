...
export const getModelName = (engine: string): string => {
  if (engine === 'arIA Flash') return 'gemini-3.6-flash';
  if (engine === 'arIA Visión') return 'gemini-3.6-flash';
  if (engine === 'arIA Pro') return 'gemini-3.1-pro-preview';
  ...
  return 'gemini-3.6-flash'; // fallback
};
